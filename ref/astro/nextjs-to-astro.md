...
Keep Next’s public/ folder untouched.
...
Copy or Move Next’s other files and folders (e.g. pages, styles etc.) into Astro’s src/ folder as you rebuild your site, following Astro’s project structure.
...
Convert any instances of getStaticProps() to either import.meta.glob() or getCollection()/getEntry() in order to access data from other files in your project source. To fetch remote data, use fetch().
